...
void MotorPositionController::setMotionMagicAngular(
    ...
    maxVel = m_pEncoder->convertAngleToTicks(maxVel) / 10.0; // convert to Controller native units
    m_pDriveMotor->ConfigMotionCruiseVelocity(maxVel, 0);
    m_pDriveMotor->Config_kF(0, kf, 0);

    maxAccel = m_pEncoder->convertAngleToTicks(maxAccel) / 10.0; // convert to Controller native units
...
void MotorPositionController::setMotionMagicLinear(
    ...
    maxVel = m_pEncoder->convertWheelDistanceToTicks(wheelRadius, maxVel) / 10.0; // convert to Controller native units
    m_pDriveMotor->ConfigMotionCruiseVelocity(maxVel, 0);
    m_pDriveMotor->Config_kF(0, kf, 0);

    maxAccel = m_pEncoder->convertWheelDistanceToTicks(wheelRadius, maxAccel) / 10.0; // convert to Controller native units
	m_pDriveMotor->ConfigMotionAcceleration(maxAccel, 0);
...
void MotorPositionController::updateAngular(double refP, double refV, double refA) {
    ...
    refV = m_pEncoder->convertAngleToTicks(refV) / 10.0; // convert to Controller native units
    refA = m_pEncoder->convertAngleToTicks(refA) / 10.0; // convert to Controller native units
    ...
    if(!m_enableMotionMagic) {
        m_pDriveMotor->Set(CommonModes::Position, refP, DemandType::DemandType_ArbitraryFeedForward, feedforwardControl);
    }
    else {
...
void MotorPositionController::updateLinear(double refP, double refV, double refA, double wheelRadius) {
    ...
    refV = m_pEncoder->convertWheelDistanceToTicks(wheelRadius, refV) / 10.0; // convert to Controller native units
    refA = m_pEncoder->convertWheelDistanceToTicks(wheelRadius, refA) / 10.0; // convert to Controller native units
