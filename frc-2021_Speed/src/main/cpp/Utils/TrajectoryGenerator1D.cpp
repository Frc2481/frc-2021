...
TrajectoryGenerator1D(
	...
	m_rangeMax(0),
	m_isContinous(false),
	m_waypointsFilename("home/lvuser/TrajectoryGenerator1DWaypoints.csv"),
	m_pathFilename("home/lvuser/TrajectoryGenerator1DFinalPath.csv") {
...
void TrajectoryGenerator1D::generatePath() {
    ...
    if(m_isContinous) {
        difference = normalizeToRange::RangedDifference(difference, m_rangeMin, m_rangeMax);
    }
    int directionSign = sign(difference);
    printf("diff %f, traj %d \n",difference, directionSign);
    ...
    printf("tempPathGenPoint.vel %f\n",tempPathGenPoint.vel);
...
void TrajectoryGenerator1D::writeTempPathToCSV() const {
    std::remove("TrajectoryGenerator1DTempPath.csv");
    
    std::ofstream myFile;
    myFile.open("TrajectoryGenerator1DTempPath.csv");
    myFile << "time (s), pos (units), dist (units), vel (units/s), accel (units/s^2)\n";
...
void TrajectoryGenerator1D::writeComboPathToCSV() const {
    std::remove("TrajectoryGenerator1DTempComboPath.csv");
    
    std::ofstream myFile;
    myFile.open("TrajectoryGenerator1DTempComboPath.csv");
    myFile << "time (s), pos (units), dist (units), vel (units/s), accel (units/s^2)\n";
