...
class TalonFXMotorController : public CommonMotorController {
 public:
  TalonFXMotorController(int motorID, const std::string &name);
  ...
  void  config_PID(const TalonFXPIDSetConfiguration& pid, int pidIdx = 0, int timeoutMs = 50);
  ...
  int GetSelectedSensorPosition(int id = 0);
